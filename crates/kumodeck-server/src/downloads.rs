...
use kumodeck_providers::DownloadJob;
...
async fn wait_until_started(manager: &DownloadManager, cancel: &CancellationToken) -> bool {
...
async fn run_audio(
    ...
    if !wait_until_started(&manager, &cancel).await {
        return;
    }
...
mod tests {
    ...
    fn the_final_suffix_corrects_the_displayed_quality() {
        ...
        let dir = std::env::temp_dir().join(format!("kumodeck-dl-{}", std::process::id()));
    ...
    async fn tasks_are_held_while_auto_start_is_off() {
        ...
        let waiter = {
            ...
            tokio::spawn(async move { wait_until_started(&manager, &cancel).await })
        };
        // 开关关着就该一直挂着
    ...
    async fn a_held_task_that_gets_cancelled_never_starts() {
        ...
        let cancel = CancellationToken::new();
        cancel.cancel();
        assert!(!wait_until_started(&manager, &cancel).await);
    }
}
