...
/// Presentation-only overlap for either media type. Nominal edit points and
/// parameters remain untouched. Keep this quadrature identical to the editor.
/// The persisted field name video_transition is retained for project compatibility.
...
impl CompositionProject {
    pub fn video_project(&self) -> Self { self.transition_project(false) }
    pub fn audio_project(&self) -> Self { self.transition_project(true) }

    fn transition_project(&self, audio: bool) -> Self {
        ...
        for layer in &mut project.layers {
            ...
            for i in 1..original.len() {
                if ![&original[i - 1], &original[i]].iter().all(|c| self.source(&c.source_id).is_some_and(|s| if audio { s.audio } else { s.video })) { continue; }
                if let Some((before, after)) = video_transition_span(&original[i - 1], &original[i]) {
                    heads[i] = Some((before, before + after));
                    tails[i - 1] = Some((after, before + after));
                }
            }
            for (i, clip) in layer.clips.iter_mut().enumerate() {
                ...
                let after = tails[i].map_or(0., |t| t.0);
                ...
                let fade_in = if audio { old.fades.audio_in_ms } else { old.fades.video_in_ms };
                let fade_out = if audio { old.fades.audio_out_ms } else { old.fades.video_out_ms };
                let visible_in = (fade_in - old.fades.offset_ms).max(0.).min(old.duration());
                let visible_out = (fade_out - (old.fades.span_ms - old.fades.offset_ms - old.duration())).max(0.).min(old.duration());
                clip.fades.offset_ms = 0.;
                clip.fades.span_ms = clip.duration();
                if audio {
                    clip.fades.audio_in_ms = heads[i].map_or(visible_in, |h| h.1);
                    clip.fades.audio_out_ms = tails[i].map_or(visible_out, |t| t.1);
                } else {
                    clip.fades.video_in_ms = heads[i].map_or(visible_in, |h| h.1);
                    // Source-over needs an opaque outgoing plane; sound instead
                    // fades both sides with complementary smoothstep weights.
