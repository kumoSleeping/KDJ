...
/// Accept bin, the extracted package root, or its immediate containing folder.
fn find_bin(root: &Path) -> Result<PathBuf> {
    ...
    for entry in fs::read_dir(root)
        .context("无法读取文件夹，请选择解压后的 FFmpeg 文件夹")?
        .take(MAX_FILES + 1)
    {
        let entry = entry?;
        if entry.file_type()?.is_dir() && has_pair(&entry.path().join("bin")) {
            found.push(entry.path().join("bin"));
        }
    }
    ensure!(
        found.len() <= 1,
        "文件夹中有多套 FFmpeg，请直接选择其中一套的文件夹"
    );
    found
        .pop()
