...
async fn main() -> anyhow::Result<()> {
    ...
    let target = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "BV1GJ411x7h7".into());
    ...
    match provider.resolve_video(&target).await {
        Ok(info) => {
            println!("\n== resolve «{}» ==", info.title);
            println!(
                "  bvid={} author={} duration={}s pages={} logged_in={}",
                info.bvid,
                info.author,
                info.duration,
                info.pages.len(),
            ...
            for option in info.options.iter().take(6) {
                println!(
                    "   qn={} {} {}p codec={}",
                    option.quality_id, option.label, option.height, option.codec
                );
            }
            ...
            match provider
                .download_video(&req, &cancel, &noop_progress())
                .await
            {
                Ok(path) => {
