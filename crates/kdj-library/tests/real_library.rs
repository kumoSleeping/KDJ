...
//!
//! 库文件路径由 `KDJ_TEST_DB` 指定；没设就跳过（CI 上没有这个文件）。
//! **只读**：测试前会先拷一份到临时目录，绝不碰用户的原始数据。

use std::path::PathBuf;

use kdj_library::service::{LibraryService, TrackQuery};
use kdj_library::Database;

fn open_real_library() -> Option<LibraryService> {
    let source = PathBuf::from(std::env::var("KDJ_TEST_DB").ok()?);
    if !source.is_file() {
        return None;
    }
...
fn camelot_sort_puts_10a_after_9a_not_before_8a() {
    ...
    let keys: Vec<(u32, char)> = page
        ...
        .filter_map(|track| kdj_library::camelot::split_camelot(&track.camelot))
        .collect();
...
fn harmonic_matches_are_compatible_sorted_and_deduped() {
    ...
    let allowed: std::collections::HashSet<String> =
        kdj_library::camelot::camelot_relations(&source.camelot, true)
            .into_iter()
            .map(|(code, _)| code)
            .collect();
