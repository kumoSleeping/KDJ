//! 真机冒烟：cargo run -p kumodeck-providers --example smoke_qq -- <关键词>
use kumodeck_core::models::Quality;
use kumodeck_providers::{qqmusic::QqMusicProvider, MusicProvider, ProviderContext};

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let dir = std::env::temp_dir().join("kumodeck-smoke-qq");
    ...
    if let Some(free) = results.iter().find(|item| !item.vip) {
        ...
        match provider.download(job).await {
            Ok(path) => {
                ...
                println!("   duration={:?}", kumodeck_providers::tags::read_duration_secs(&path));
            }
            Err(err) => println!("\n== download 失败：{err:#} =="),
        }
    }
