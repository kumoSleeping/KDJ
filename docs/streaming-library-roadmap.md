...
- 已落地三类左侧平台根节点与“我的收藏”入口；网易云登录态可读取远程歌单，其它平台在没有稳定登录/收藏接口时保留本地流媒体收藏，不伪造远程数据。
- 已落地在线流播放音质设置、B 站预览画质设置、下一首在线流 URL 预加载。
- 尚未完成：Finder/资源管理器原生文件拖入的 Tauri drop 路径导入、QQ/SoundCloud 远程收藏的稳定认证链、远程歌单的分页/缓存和流媒体 Track 的独立波形/离线缓存。这些保留为下一阶段，不影响已完成的来源链。
