...
(async()=>{
  ...
  const full=path.join(out,'KDJ-hand-painted-2400x3600.png');
  await page.screenshot({path:full,omitBackground:true});
  await sharp(full).resize(1200,1800).png().toFile(path.join(out,'KDJ-hand-painted-preview.png'));
  ...
  fs.writeFileSync(path.join(out,'KDJ-hand-painted-layered.psd'),concat([header,u32(0),u32(0),u32(section.length),section,u16(0),...merged]));
  ...
  console.log('Exported PNG, preview, 11 layer PNGs and layered PSD.');
