...
(async()=>{
  ...
  const full=path.join(out,'KDJ-ink-light-2400x3600.png');
  await page.screenshot({path:full,omitBackground:true});
  await sharp(full).resize(1200,1800).png().toFile(path.join(out,'KDJ-ink-light-preview.png'));
  ...
  fs.writeFileSync(path.join(out,'KDJ-ink-light-layered.psd'),concat([header,u32(0),u32(0),u32(section.length),section,u16(0),...merged]));
  ...
  console.log('Exported PNG, preview, 10 layer PNGs and layered PSD.');
