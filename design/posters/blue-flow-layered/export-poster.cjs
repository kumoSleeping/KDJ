...
async function makeLayer(file,name){
  ...
  const record=concat([u32(t),u32(l),u32(b),u32(r),u16(4),...channels.map((c,i)=>concat([i16(i===3?-1:i),u32(c.length)])),Buffer.from('8BIMnorm'),Buffer.from([255,0,0,0]),u32(extra.length),extra]);
...
(async()=>{
  ...
  const full=path.join(out,'KDJ-blue-flow-2400x3600.png');
  await page.screenshot({path:full,omitBackground:true});
  await sharp(full).resize(1200,1800).png().toFile(path.join(out,'KDJ-blue-flow-preview.png'));
  const names=['01 Paper','02 Atmosphere','03 Grid details','04 Sound orbit','05 KDJ custom lettering','06 Back ribbons','07 Character - manual color','08 KDJ foreground outline','09 Chinese headline','10 Front ribbons','11 Foreground light','12 Flow typography','13 Product copy','14 Waveform','15 Header and footer'];
  const layerFiles=[];
  ...
  fs.writeFileSync(path.join(out,'KDJ-blue-flow-layered.psd'),concat([header,u32(0),u32(0),u32(section.length),section,u16(0),...merged]));
  ...
  console.log('Exported PNG, preview, 15 layer PNGs and layered PSD.');
