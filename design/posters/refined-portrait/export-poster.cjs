...
(async()=>{
  ...
  const full=path.join(out,'KDJ-refined-portrait-2400x3600.png');
  await page.screenshot({path:full,omitBackground:true});
  await sharp(full).resize(1200,1800).png().toFile(path.join(out,'KDJ-refined-portrait-preview.png'));
  if(process.argv.includes('--preview-only')){await browser.close();console.log('Preview exported.');return;}
  ...
  layers.reverse(); // PSD stores the topmost layer first.
  ...
  fs.writeFileSync(path.join(out,'KDJ-refined-portrait-layered.psd'),concat([header,u32(0),u32(0),u32(section.length),section,u16(0),...merged]));
