            ...
            Ok(plaintext) => {
                ...
                let plaintext = if key_type == b"v20" {
                    String::from_utf8(plaintext[32..].to_vec())
                        .context("Can't decode encrypted value")
                } else {
        ...
        if let Ok(plaintext) = cipher.decrypt_padded_mut::<Pkcs7>(&mut cloned_encrypted_value) {
            ...
            match decoded {
                ...
                Err(_) => {
                    ...
                    let decoded =
                        String::from_utf8(plaintext[32..].to_vec()).unwrap_or_else(|_| {
...
fn unlock_file(mut path: PathBuf) -> Result<PathBuf> {
    ...
    if privilege::user::privileged() {
        ...
        if let Ok(temp_dir) = windows::shadow_copy::temp_folder(".tmp", "", 10) {
            let result =
                windows::shadow_copy::shadow_copy(path.clone(), temp_dir.clone().to_path_buf());
