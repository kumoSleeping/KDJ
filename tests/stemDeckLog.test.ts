...
function model(patch: Partial<StemModelStatus> = {}): StemModelStatus {
  return {
    ...
    version: "ZF-v1.0.6+deploy-v0.1.2",
    supported: true,
    state: "ready",
    progress: 1,
    downloadedBytes: 1,
...
test("runtime line names the model and the selected CPU/GPU path", () => {
  ...
  assert.equal(ready.text, "SCNet Small ZF-v1.0.6+deploy-v0.1.2 · Core ML · Apple GPU · F 12ms · B 9ms · P95 11ms");
  ...
  assert.equal(pending.text, "SCNet Small ZF-v1.0.6+deploy-v0.1.2 · Core ML");
});
...
test("deck log keeps two independent lines and surfaces errors", () => {
  ...
  assert.equal(log.runtime, "SCNet Small ZF-v1.0.6+deploy-v0.1.2 · Core ML · Apple GPU · F 12ms · B 9ms · P95 11ms");
